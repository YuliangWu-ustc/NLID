...
def fit_regressor(x_train, y_train, x_test, y_test,  # 函数名改为更合适的fit_regressor
                 regressor_name, output_directory,   # 参数名改为regressor_name
    ...
    # 移除所有分类相关的处理
    # 直接使用原始连续标签
    y_true = y_test  # 直接使用原始测试标签作为真实值

    # 保持输入形状调整
    if len(x_train.shape) == 2:  # 单变量数据转多变量格式
    ...
    # 创建回归器实例
    regressor = create_regressor(regressor_name, input_shape, output_directory, device)
    
    # 修改训练调用参数（移除nb_classes相关参数）
    regressor.fit(x_train, y_train, x_test, y_test, y_true, nb_epochs=epochs, y_labels_description=y_labels_description)

def create_regressor(regressor_name, input_shape, output_directory, verbose=True, device='cuda'):
    """
    创建回归器实例的工厂函数
    """
    if regressor_name == 'cnnatt-regr':
        from classifiers import cnnatt_regr
        # 修改为回归器类并移除nb_classes参数
        return cnnatt_regr.Regressor_CNN_LSTM_Torch(output_directory, input_shape, verbose, device)
    # 其他模型适配...
    # 添加更多回归模型的创建逻辑
    else:
...
if __name__ == '__main__':
    ...
    regressor_name = 'cnnatt-regr'  # 变量名改为更合适的regressor_name
    ...
    output_directory = './results/' + regressor_name + '/' + archive_name + '/' + \
                       dataset_name + '/' + str(SELECTED_AREAS[0]) + str(DELTA_T) + '/'
    
    print('Method: ', archive_name, dataset_name, regressor_name)
    
    create_directory(output_directory)
    
    # # 假设read_dataset_torch返回的y已经是连续值
    # x_train, y_train, x_test, y_test = read_dataset_torch(
    #     dataset_path, archive_name, dataset_name, 
    #     folder_labels_dict=folder_labels_dict, 
    ...
    # 调用修改后的回归训练函数
    fit_regressor(x_train, y_train.astype(np.float32),  # 确保标签是浮点类型
                 x_test, y_test.astype(np.float32),     # 适用于回归任务
                 regressor_name, output_directory,
                 epochs=600, 
                 y_labels_description=list(folder_labels_dict.keys()), 
                 device=device)
    # 调用修改后的回归训练函数
    # fit_regressor(x_train, y_train.astype(np.float32),  # 确保标签是浮点类型
    #              x_test, y_test.astype(np.float32),     # 适用于回归任务
    ...
    print(f'{SELECTED_AREAS[0]} {DELTA_T} DONE')