...
class Classifier_MLP:
    ...
    def __init__(self, output_directory, input_shape, nb_classes, verbose=False,build=True):
        ...
        if build == True:
            self.model = self.build_model(input_shape, nb_classes)  # ⭐ 构建MLP模型结构
            if(verbose==True):
                self.model.summary()
            self.verbose = verbose
            self.model.save_weights(self.output_directory + 'model_init.hdf5')  # ⭐ 保存初始模型权重
        return
