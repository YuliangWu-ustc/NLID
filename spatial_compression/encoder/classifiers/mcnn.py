...
class Classifier_MCNN:
    ...
    def __init__(self, output_directory, verbose, build=True):
        ...
        Args:
            ...
            build (bool, optional): 是否立即构建模型. 默认为True
        """
        self.output_directory = output_directory
        self.verbose = verbose
        self.pool_factors = [2,3,5] # used for hyperparameters grid search  # ⭐ 定义池化因子超参数搜索空间
        self.filter_sizes = [0.05,0.1,0.2] # used for hyperparameters grid search 

    def slice_data(self, data_x, data_y, slice_ratio): 
        """
        对时间序列数据进行切片处理，生成更多训练样本
        
        Args:
            data_x (np.array): 输入时间序列数据，形状为(n_samples, seq_length, n_features)
            data_y (np.array): 对应的标签数据，形状为(n_samples, n_classes)
            slice_ratio (float): 切片比例(0-1之间)
            
        Returns:
            tuple: (new_x, new_y) 切片后的新样本和对应标签
        """
        n = data_x.shape[0]
        ...
        for i in range(n):
            for j in range(increase_num):
                new_x[i * increase_num + j, :,:] = data_x[i,j : j + length_sliced,:]  # ⭐ 执行数据切片操作
