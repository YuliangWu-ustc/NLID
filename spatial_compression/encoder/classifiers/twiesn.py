...
class Classifier_TWIESN:
    """
    时间加权回声状态网络(TWIESN)分类器实现
    
    该类实现了基于回声状态网络的时间序列分类算法，通过随机矩阵生成特征空间，
    并结合岭回归进行模型训练和预测。
    
    Attributes:
        output_directory (str): 输出结果保存路径
        verbose (bool): 是否打印详细日志
        configs (list): 网络配置参数列表，包含4种不同配置
        rho_s (list): 谱半径参数列表，用于控制网络动态特性
        alpha (float): 泄漏率参数，控制网络状态更新速度
    """
    ...
    def __init__(self, output_directory,verbose): 
        ...
        Args:
            output_directory (str): 结果输出目录路径
            verbose (bool): 是否显示详细运行信息
        """
        self.output_directory = output_directory
        self.verbose = verbose
        # hyperparameters 
        first_config = {'N_x':250,'connect':0.5,'scaleW_in':1.0,'lamda':0.0}  # ⭐ 定义第一种网络配置参数
        ...
        self.rho_s = [0.55,0.9,2.0,5.0]  # ⭐ 定义谱半径参数列表，控制网络动态范围
