...
class Classifier_TLENET:
    ...
    def __init__(self, output_directory, verbose,build=True):
        ...
        self.warping_ratios = [0.5,1,2]  # ⭐ 设置时间扭曲的比例参数
    ...
    def slice_data(self, data_x, data_y, length_sliced): 
        ...
        Args:
            ...
            data_y (np.array): 对应标签数据，形状为(n_samples, n_classes)
            length_sliced (int): 切片后的序列长度

        Returns:
            tuple: (增强后的数据new_x, 对应标签new_y, 每个样本生成的切片数量increase_num)
        """
        n = data_x.shape[0]
        ...
        for i in range(n):
            for j in range(increase_num):
                new_x[i * increase_num + j, :,:] = data_x[i,j : j + length_sliced,:]  # ⭐ 核心切片操作
