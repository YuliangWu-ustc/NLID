...
# 随机初始化的迭代次数
ITERATIONS = 5  # ⭐ 控制模型随机初始化的重复次数，用于确保结果稳定性

# 时间序列数据集的归档名称
ARCHIVE_NAMES = ['UCRArchive_2018']

# 归档名称与对应数据集名称的映射
dataset_names_for_archive = {'UCRArchive_2018': UNIVARIATE_DATASET_NAMES_2018}  # ⭐ 关键映射关系，连接归档名和具体数据集

# 可用的分类器类型列表
CLASSIFIERS = ['fcn', 'mlp', 'resnet', 'tlenet', 'mcnn', 'twiesn', 'encoder', 'mcdcnn', 'cnn', 'inception']

# 数据集类型映射字典（将数据集名称映射到其所属领域）
