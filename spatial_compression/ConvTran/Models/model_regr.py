...
def model_factory(config):
    ...
    根据配置参数创建对应的神经网络模型实例。
    Args:
        config (dict): 包含网络类型和输出维度等参数的配置字典。
            - Net_Type: 网络类型标识(T/CC-T/其他)
            - output_dim: 回归任务的输出维度（默认为1）
    Returns:
        nn.Module: 创建的神经网络模型实例
    """
    output_dim = config.get('output_dim', 1)  # 默认输出维度为1
    if config['Net_Type'][0] == 'T':
        model = Transformer(config, num_classes=output_dim)
    elif config['Net_Type'][0] == 'CC-T':
        model = CasualConvTran(config, num_classes=output_dim)
    else:
        model = ConvTran(config, num_classes=output_dim)
    return model
...
class ConvTran(nn.Module):
    def __init__(self, config, num_classes):
        ...
        # Embedding Layer -----------------------------------------------------------
        self.embed_layer = nn.Sequential(
                    nn.Conv2d(1, emb_size*4, kernel_size=[1,7], padding='same'),  # 奇数核方案
