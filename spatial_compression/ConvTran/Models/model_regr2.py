...
def model_factory(config):
    ...
    Args:
        config (dict): 包含网络类型(num_labels等参数的配置字典。
            - Net_Type: 网络类型标识(T/CC-T/其他)
            - num_labels: 分类任务的类别数
    ...
    if config['Net_Type'][0] == 'T':
        model = Transformer(config, num_classes=config['num_labels'])  # ⭐ 创建标准Transformer模型
    elif config['Net_Type'][0] == 'CC-T':
        model = CasualConvTran(config, num_classes=config['num_labels'])  # ⭐ 创建因果卷积Transformer混合模型
    else:
        model = ConvTran(config, num_classes=config['num_labels'])  # ⭐ 创建卷积Transformer混合模型
    return model
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.Fix_pos_encode != 'None' and self.Fix_Position is not None:
        ...
        # out = self.out(out[-1])
        # print(f'in forward, out.shape: {out.shape}')
...
class CausalConv1d(nn.Conv1d):
    ...
    def forward(self, x):
        # print(f'in CausalConv1d, x.shape: {x.shape}')
