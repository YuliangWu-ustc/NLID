...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x (torch.Tensor): 输入张量，形状为(batch_size, seq_len, emb_size)
            
        Returns:
            torch.Tensor: 经过注意力计算后的输出张量
        """
        batch_size, seq_len, _ = x.shape
        k = self.key(x).reshape(batch_size, seq_len, self.num_heads, -1).permute(0, 2, 3, 1)  # ⭐ 计算并重塑key张量
        ...
        out = torch.matmul(attn, v)  # ⭐ 注意力权重与value相乘得到输出
...
class Attention_Rel_Scl(nn.Module):
    def __init__(self, emb_size, num_heads, seq_len, dropout):
        ...
        self.relative_bias_table = nn.Parameter(torch.zeros((2 * self.seq_len - 1), num_heads))  # ⭐ 初始化相对位置偏置表
