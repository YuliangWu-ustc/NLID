...
def accuracy(y_pred, y_true, excluded_labels=None):
    ...
    计算预测结果的准确率，可选择排除特定标签的计算

    Args:
        y_pred (array-like): 预测标签数组
        y_true (array-like): 真实标签数组
        excluded_labels (list, optional): 需要排除的标签列表. 默认为None

    Returns:
        float: 计算得到的准确率值(0-1之间)
    """

    if excluded_labels is None:
        return np.mean(y_pred == y_true)  # ⭐ 基础准确率计算(不排除任何标签)
    else:
        ...
        return np.mean(y_pred[included] == y_true[included])  # ⭐ 排除指定标签后的准确率计算
...
def precision(y_true, y_pred, label):
    ...
    Args:
        y_true (array-like): 真实标签数组
        y_pred (array-like): 预测标签数组
        label (int): 要计算精确率的类别索引

    Returns:
        float: 指定类别的精确率值，范围[0,1]
    """
    predicted_in_C = (y_pred == label)  # ⭐ 找出所有预测为该类别的样本索引
...
def recall(y_true, y_pred, label):
    ...
    """
    计算并返回指定类别的召回率(Recall)

    Args:
        y_true (array-like): 真实标签数组
        y_pred (array-like): 预测标签数组
        label (int/str): 要计算召回率的特定类别标签

    Returns:
        float: 指定类别的召回率值，范围[0,1]。如果没有该类别样本则返回0
    """
    truly_in_C = (y_true == label)  # ⭐ 找出真实标签中属于指定类别的样本
    num_truly_in_C = np.sum(truly_in_C)
    if num_truly_in_C == 0:
        return 0  # or NaN?
    return np.sum(y_pred[truly_in_C] == label) / num_truly_in_C  # ⭐ 计算召回率核心公式
...
def limiter(metric_functions, y_true, y_pred, y_scores, score_thr, label):
    ...
    对一组指标函数进行包装处理，忽略特定类别下低于置信度阈值的预测结果。

    Args:
        metric_functions (list): 包含需要计算的指标函数列表（如精确度、召回率等）
        y_true (array): 真实标签数组
        y_pred (array): 预测标签数组
        y_scores (array): 预测置信度分数数组
        score_thr (float): 置信度阈值，低于此值的预测将被忽略
        label (int): 需要特别处理的类别标签

    Returns:
        list: 经过阈值过滤后的各指标计算结果列表
    """

    ltd_pred = np.copy(y_pred)
    ltd_pred[(ltd_pred == label) & (y_scores < score_thr)] = -1  # ⭐ 核心操作：将低于阈值的特定类别预测标记为-1（忽略）
...
def prec_rec_parametrized_by_thr(y_true, y_pred, y_scores, label, Npoints, min_score=None, max_score=None):
    ...
    if (min_score is None) or (max_score is None):
        ...
        min_score = 0.99 * np.amin(y_scores[predicted_in_C])  # ⭐ 自动计算最小分数阈值，确保保留所有预测
        max_score = 1.01 * np.amax(y_scores[predicted_in_C])  # ⭐ 自动计算最大分数阈值，确保不保留任何预测

    grid = np.linspace(min_score, max_score, Npoints)

    measure = lambda x: limiter([precision, recall], y_true, y_pred, y_scores, x, label)  # ⭐ 核心计算函数：计算每个阈值点的precision和recall
...
def plot_prec_vs_rec(score_grid, rec, prec, prec_requirement=None, thr_opt=None, title=None, show=True, save_as=None):
    ...
    plt.hold(True)
    l_prec, = plt.plot(score_grid, prec, 'g.-')  # ⭐ 绘制精确度曲线
    plt.ylim((0, 1.01))
    ...
    if not (thr_opt is None):
        l_score_thr = plt.axvline(thr_opt, color='r')  # ⭐ 绘制最优阈值线
...
def plot_confusion_matrix(ConfMat, label_strings=None, title='Confusion matrix', cmap=plt.cm.get_cmap('Blues')):
    ...
    plt.imshow(ConfMat, interpolation='nearest', cmap=cmap)  # ⭐ 核心绘图函数：绘制混淆矩阵热力图
    plt.title(title)
    plt.colorbar()
    if label_strings:
...
class Analyzer(object):
    ...
    def analyze_regression(self, predictions, targets):
        """
        分析回归任务的预测结果，计算MSE、MAE、MaxAE和R²等指标。

        Args:
            predictions (np.ndarray): 模型预测值
            targets (np.ndarray): 真实标签值

        Returns:
            dict: 包含MSE、MAE、MaxAE和R²等指标的字典
        """
        # 计算均方误差 (MSE)
        ...
        # 计算最大绝对误差 (MaxAE)
        maxae = np.max(np.abs(predictions - targets))
