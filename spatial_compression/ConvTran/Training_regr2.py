...
from Models.loss_regr2 import l2_reg_loss
from Models import utils, analysis_regr2
...
class SupervisedTrainer(BaseTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(args[3], torch.nn.CrossEntropyLoss):
            ...
            self.analyzer = analysis_regr2.Analyzer(print_conf_mat=False)
        else:
            self.classification = False
            self.analyzer = analysis_regr2.Analyzer(print_conf_mat=True)
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            # print(f'in train_epoch, predictions.shape: {predictions.shape}')
            # print(f'in train_epoch, targets.shape: {targets.shape}')
            
            loss = self.loss_module(predictions, targets)  # (batch_size,) loss for each sample in the batch
            # print(f'in train_epoch, loss.shape: {loss.shape}')
    ...
    def evaluate(self, epoch_num=None, keep_all=True, return_pred_targets=False):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            # print(f'in evaluate, predictions.shape: {predictions.shape}')
            # print(f'in evaluate, targets.shape: {targets.shape}')
            
            loss = self.loss_module(predictions, targets)  # (batch_size,) loss for each sample in the batch
            
            # print(f'in evaluate, loss.shape: {loss.shape}')
        ...
        metrics_dict = self.analyzer.analyze_regression(predictions, targets)  # 修改为回归任务的评估

        self.epoch_metrics['mse'] = metrics_dict['mse']  # 均方误差
        self.epoch_metrics['mae'] = metrics_dict['mae']  # 平均绝对误差
        self.epoch_metrics['maxae'] = metrics_dict['maxae']  # 最大绝对误差
        self.epoch_metrics['r2'] = metrics_dict['r2']  # R2
...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    # print(f'in validate, aggr_metrics: {aggr_metrics}'  )
    # print(config['key_metric'])
