tag_this_file = '''\neval best model clsf 1/2, 10000, test=raw(no aug)'''
print(f'==============================================')
...
best_model_path = r'./Results/classification_(530, 270, 810, 540)_10000/2025-05-15_09-03/Dataset/UEA/checkpoints/classificationmodel_last.pth'
...
# ----------------------------------------------------------------------------------------------------------------------
# ------------------------------------------------------ System --------------------------------------------------------
parser.add_argument('--gpu', type=int, default=device, help='GPU index, -1 for CPU')
parser.add_argument('--console', action='store_true', help="Optimize printout for console output; otherwise for file")
parser.add_argument('--seed', default=seed, type=int, help='Seed used for splitting sets')
...
def downsample_max(data, factor=10):
    """
    对数据进行降采样，使用最大值的方式。如果序列长度不能被降采样因子整除，则进行边缘填充。

    Args:
        data (np.ndarray): 输入数据，形状为 (num_samples, seq_length)
        factor (int): 降采样因子，默认为10

    Returns:
        np.ndarray: 降采样后的数据，形状为 (num_samples, ceil(seq_length / factor))
    """
    num_samples, seq_length = data.shape
    
    # 如果序列长度不能被降采样因子整除，进行边缘填充
    if seq_length % factor != 0:
        padding_length = factor - (seq_length % factor)
        # 使用边缘填充（复制最后一个值）
        data = np.pad(data, ((0, 0), (0, padding_length)), mode='edge')
        seq_length += padding_length
    
    # 将数据重塑为 (num_samples, seq_length // factor, factor)
    reshaped_data = data.reshape(num_samples, -1, factor)
...
def my_TSC_data_loader(dataset_path, folder_labels_dict, selected_areas, delta_t):
    ...
    # 对训练数据和测试数据进行降采样
    train_ev_freq_series_list = downsample_max(train_ev_freq_series_list, factor=10)
    test_ev_freq_series_list = downsample_max(test_ev_freq_series_list, factor=10)

    # 打印降采样后的形状
    print(f'降采样后 train_ev_freq_series_list.shape: {train_ev_freq_series_list.shape}')
    print(f'降采样后 test_ev_freq_series_list.shape: {test_ev_freq_series_list.shape}')
...
if __name__ == '__main__':
    ...
    # 移除了测试集的加载
    X_train, y_train, X_val, y_val = my_TSC_data_loader(dataset_path, folder_labels_dict, selected_area_list, DELTA_T)
    ...
    best_val_evaluator = SupervisedTrainer(best_model, val_loader, device, config['loss_module'],
                                            print_interval=config['print_interval'], console=config['console'],
                                            print_conf_mat=True)
    best_aggr_metrics_val, all_metrics, predictions, targets = best_val_evaluator.evaluate(keep_all=True, return_pred_labels=True)
    ...
    np.save(os.path.join(best_output_dir, 'predictions.npy'), predictions)
    ...
    print(f'{selected_area_list[0]} {DELTA_T} {problem} {All_Results_df} {tag_this_file}')
